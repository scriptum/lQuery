...
local funcOrTable = function(func, a, b, c, d)
	if type(func) == 'function' then
		func(a, b, c, d)
...
local function events(v)
	if lQuery.KeyPressed == true then 
		if v._keypress then
			if not v._key or v._key ~= lQuery.KeyPressedKey then
				funcOrTable(v._keypress, v, lQuery.KeyPressedKey, lQuery.KeyPressedUni)
			end
		end
		if not v._key or v._key ~= lQuery.KeyPressedKey then
		...
		if v._keyrepeat and (v._KeyPressedCounter == 1 or 
			...
			funcOrTable(v._keyrepeat, v, lQuery.KeyPressedKey, lQuery.KeyPressedUni)
		end
		v._key = lQuery.KeyPressedKey
	else
		if v._keyrelease then
			if v._key and v._key == true then
				funcOrTable(v._keyrelease, v, lQuery.KeyPressedKey, lQuery.KeyPressedUni)
			end
		end
		v._key = false
	end
	if v._bound and v._bound(v, mX, mY) then 
		if v._mousemove then 
			funcOrTable(v._mousemove, v, mX, mY)
		end 
		if lQuery.MouseButton == "wu" then 
			if v._wheel then
				lQuery.MouseButton = nil
				funcOrTable(v._wheel, v, mX, mY, "u")
			end
		elseif lQuery.MouseButton == "wd" then 
			if v._wheel then
				lQuery.MouseButton = nil
				funcOrTable(v._wheel, v, mX, mY, "d")
			end
		...
		if lQuery._hover == v then 
			lQuery._hover = nil
			lQuery.hover = nil
			if v._mouseout then funcOrTable(v._mouseout, v, mX, mY) end
...
local function process_entities(ent)
	if ent._visible == true then 
		...
		if ent._draw then lQuery.draw(ent) end
		if ent._child then 
	...
	elseif e == "mr" then 
		...
		if v --[[and v._bound and v._bound(v, mX, mY)]] then
			...
			if v._mouserelease then 
				funcOrTable(v._mouserelease, v, mX, mY, c)
			end
			if v._click and v._bound and v._bound(v, mX, mY) then 
				funcOrTable(v._click, v, mX, mY, c)
			end
		end
...
lQuery.process = function()
	...
	if v and lQuery._MousePressedOwner == true then
		if v._mousepress then 
			funcOrTable(v._mousepress, v, mX, mY, lQuery.MouseButton)
		end
	end
	
	local v = lQuery.hover
	if v and not lQuery._hover then
		if v._mouseover then funcOrTable(v._mouseover, v, mX, mY) end
		lQuery._hover = v
	end
